...
const bgStoreDetailsModule = require("./Modal/bgStoreDetails");
const bgStoreDetails =
  bgStoreDetailsModule?.bgStoreDetails || bgStoreDetailsModule;
...
const ioServer = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("call-user", async ({ callerId, receiverId, channelName, callType, shop }) => {
      console.log("+data_______________________✅",callerId, receiverId, channelName, callType, shop )
        try {
          ...
          const user_ = await User.findById(callerId).select(
    ...
    socket.on(
      ...
      async ({
        ...
        console.log(
          "✅ time updated after both users joined:",
          cleanTransactionId,
        );
      },
    );
    ...
    socket.on("call-ended", async (data) => {
      ...
      try {
        ...
        if (transaction.status === "completed") {
          console.log(
            `⚠️ Transaction ${trnaID} became completed before processing. Rolling back.`,
          );
